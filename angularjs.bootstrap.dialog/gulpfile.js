...
const demoCssDir = demoDir + 'css/';
const demoFontsDir = demoDir + 'fonts/';
...
gulp.task('default', function() {
  console.log('胡辉煜的angularjs的bootstrap对话框');
});
...
gulp.task('demo', ['jsmin'], function() {
  //第三方库
  del.sync([demoCssDir + '**/*', demoJsDir + '**/*'], {
    force: true
  });
  sync(distDir, demoJsDir);
  sync(nodeModulesDir + 'bootstrap3/dist/fonts/', demoFontsDir);
  gulp.src(nodeModulesDir + 'bootstrap3/dist/css/bootstrap.min.css').pipe(gulp.dest(demoCssDir));
  gulp.src(nodeModulesDir + 'jquery/dist/jquery.min.js').pipe(gulp.dest(demoJsDir));
  gulp.src(nodeModulesDir + 'bootstrap3/dist/js/bootstrap.min.js').pipe(gulp.dest(demoJsDir));
